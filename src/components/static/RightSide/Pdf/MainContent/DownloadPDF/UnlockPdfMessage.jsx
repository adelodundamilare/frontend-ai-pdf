...
const UnlockPdfMessage = ({ compressedFileUrl, mergeID, onClose }) => {
  ...
  const DeleteMergePdf = async () => {
    try {
      const response = await authRequest.delete(`/pdf/unlock_pdf/delete/${mergeID}/`);

     
      if (response.status === 204) {
        toast.success('Unlock PDF deleted successfully');
        // Add any additional logic you want to execute after successful deletion
      } else {
        toast.error('Error deleting Unlock PDF not found:', response.status, response.statusText);
      }
    } catch (error) {
      toast.error('Error deleting Unlock PDF:', error.message);
    }
  };
  ...
  return (
    ...
    <div className=" flex justify-center items-center flex-col font-roboto mt-10">
                <h1 className="text-[#303030] text-lg mb-3 text-center">
                  PDFs have been Unlocked!
                </h1>
                <p className="text-[#474747] text-sm mb-3 text-center">
                  Click on download button to download Unlocked pdf or continue to
...
export default UnlockPdfMessage;
