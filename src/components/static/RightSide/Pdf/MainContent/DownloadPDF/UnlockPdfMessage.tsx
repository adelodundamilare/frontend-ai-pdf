...
const UnlockPdfMessage = ({ compressedFileUrl, mergeID, onClose }: any) => {
  ...
  const DeleteMergePdf = async () => {
    try {
      ...
      if (response.status === 204) {
        ...
        toast.error("Error deleting Unlock PDF not found:", response.status);
      }
    } catch (error: any) {
    ...
    <div className="flex justify-center items-center h-screen">
      <div className=" flex justify-center items-center flex-col font-roboto mt-10">
        ...
        <p className="text-[#474747] text-sm mb-3 text-center">
          Click on download button to download Unlocked pdf or continue to work
