...
const ProtectPdfMessage = ({ protectFileUrl, mergeID, onClose }) => {
  ...
  const DeleteMergePdf = async () => {
    try {
      const response = await authRequest.delete(`/pdf/protect_pdf/delete/${mergeID}/`);
      ...
      if (response.status === 204 || response.status === 200) {
        ...
        toast.error('Error deleting protect PDF:', response.status, response.statusText);
      }
    } catch (error) {
  ...
  return (
    ...
    <div className=" flex justify-center items-center flex-col font-roboto mt-10">
                ...
                <p className="text-[#474747] text-sm mb-3 text-center">
                  Click on download button to download protected pdf or continue to
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button className="bg-[#20808D] text-white p-2 rounded-md w-[13rem]"  onClick={downloadProtectPdf}>
                    Download to device
                  </button>
