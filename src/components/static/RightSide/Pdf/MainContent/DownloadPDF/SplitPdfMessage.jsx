...
const SplitPdfMessage = ({ splitedFileUrl, mergeID, onClose }) => {
  ...
  const DeleteMergePdf = async () => {
    try {
      const response = await authRequest.delete(`/pdf/split_pdf/delete/${mergeID}/`);
      ...
      if (response.ok) {
        ...
        toast.error('Error deleting Split PDF12:', response.status, response.statusText);
      }
    } catch (error) {
  ...
  return (
    ...
    <div className=" flex justify-center items-center flex-col font-roboto mt-10">
                ...
                <p className="text-[#474747] text-sm mb-3 text-center">
                  Click on download button to download split pdf or continue to
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button className="bg-[#20808D] text-white p-2 rounded-md w-[13rem]"  onClick={downloadSplitPdf}>
                    Download to device
                  </button>
