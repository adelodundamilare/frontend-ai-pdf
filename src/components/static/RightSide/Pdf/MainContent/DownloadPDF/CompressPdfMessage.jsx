...
const CompressPdfMessage = ({ compressedFileUrl, mergeID, onClose }) => {
  ...
  const DeleteMergePdf = async () => {
    try {
      const response = await authRequest.delete(`/pdf/compress_pdf/delete/${mergeID}/`);
      ...
      if (response.ok) {
        ...
        toast.error('Error deleting Compress PDF:', response.status, response.statusText);
      }
    } catch (error) {
  ...
  return (
    ...
    <div className=" flex justify-center items-center flex-col font-roboto mt-10">
                ...
                <p className="text-[#474747] text-sm mb-3 text-center">
                  Click on download button to download Compressed pdf or continue to
