...
const SubscriptionPopup = ({ setshowSubscriptionPopup }) => {
  ...
  useEffect(() => {
    const fetchSubscriptionPlans = async () => {
      ...
      try {
        if (token) {
          const response = await axios(`${BASE_URL}/payment/subscription_list/`, {
            method: 'GET',
            headers: {
  ...
  return (
    ...
    <div className=' bg-white font-roboto p-3 rounded-md md:w-[50vw] sm:w-[80vw] w-[98vw]'>
      ...
      <img src={CrossIcon} alt="" srcset="" className=' float-right cursor-pointer' onClick={Back} />
      ...
      {subscriptionPlans.map(plan => (
        ...
        <div key={plan.id} style={{border:"1px solid #D9D9D9"}} className='flex-1 h-[12rem] relative rounded-[0.375rem] p-3'>
          <p className='text-[#20808D] mb-3 sm:text-base text-sm'>{plan.duration}</p>
          <p className='mb-3 sm:text-base text-sm'>${convertCentsToDollars(plan.price)}</p>
          <p className='text-[#7070778] mb-3 sm:text-base text-sm'>Billed per month</p>
          <div className='absolute bottom-4 right-2 left-2'>
            <button  onClick={() => handleSelectPayment(plan.id, plan.duration, plan.price)} className='bg-[#20808D] w-[100%] h-[2.4rem] rounded-md text-white'>Get Started</button>
          </div>
        </div>
      ))}
